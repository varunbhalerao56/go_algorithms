...
func main() {
	...
	getProductOfAllOtherElements([]int{4, 5, 1, 8, 2})
}
...
func getProductOfAllOtherElements(inputArray []int) []int {
	...
	for i := 1; i < inputLen; i++ {
		left_products[i] = left_products[i-1] * inputArray[i-1]
	}

	for j := inputLen - 2; j >= 0; j-- {

		right_product[j] = right_product[j+1] * inputArray[j+1]
	}

	for m := 0; m < inputLen; m++ {

		outputArray[m] = left_products[inputLen-1-m] * right_product[inputLen-1-m]
	}
